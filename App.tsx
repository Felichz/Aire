...
const COLORS = {
  ...
  bgMid: '#0e0e2a',
  bgBottom: '#0a0a1e',
  bgGradient: '#0e0e2a', // Added missing define
...
function SkiaParticles({
  ...
  useEffect(() => {
    anim.value = withRepeat(
      withTiming(1, { duration: 25000, easing: ReanimatedEasing.linear }),
      -1,
      false
    );
  }, []);
...
function GlassCard({ children, isActive, accent, showGlow = true }: { children: React.ReactNode; isActive: boolean; accent: string; showGlow?: boolean }) {
  ...
  return (
    <View
      ...
      style={[
        ...
        {
          backgroundColor: isActive ? COLORS.cardActive : COLORS.card,
...
function AnimatedNumber({ value, color, style }: { value: number; color: any; style: any }) {
  ...
  const oldStyle = useAnimatedStyle(() => ({
    ...
    transform: [{ translateY: -25 * progress.value }], // Increased distance
    color: typeof color === 'string' ? color : color.value,
  }));

  const newStyle = useAnimatedStyle(() => ({
    opacity: progress.value,
    transform: [{ translateY: 25 * (1 - progress.value) }], // Increased distance
    color: typeof color === 'string' ? color : color.value,
  }));
...
function PhaseCard({
  ...
  return (
    ...
    >
      <GlassCard isActive={isActive} accent={phase.accent}>
        {/* Step badge */}
        ...
        <Text style={[styles.cardLabel, { color: isActive ? phase.accent : COLORS.textSecondary }]}>
          {phase.label}
        </Text>
        ...
        {isEditing ? (
          ...
          <View style={styles.secondsRow}>
            <Text style={[styles.cardSecondsNum, { color: isActive ? phase.accent : COLORS.text }]}>{seconds}</Text>
            <Text style={[styles.cardSecondsUnit, { color: isActive ? `${phase.accent}70` : COLORS.textMuted }]}>s</Text>
          </View>
        )}
      </GlassCard>
    </Animated.View>
...
function AppContent() {
  ...
  const startPhase = useCallback((phase: number, rep: number) => {
    if (rep >= repetitions) {
      ...
      setTimeout(() => {
        Animated.timing(completeOpacity, { toValue: 0, duration: 500, useNativeDriver: true }).start(() => setShowComplete(false));
      }, 4000);
      return;
    }
  ...
  }, [durations, repetitions, scrollToPhase, startBreatheAnimation]);
  ...
  const handlePlay = useCallback(() => {
    ...
    if (isRunning) {
      ...
      breatheOpacity.value = withTiming(0.7, { duration: 100 }); // Start at 70%
      particleScale.value = withTiming(1.3, { duration: 100 });

      startPhase(0, 0);
      if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    }
  }, [isRunning, handleStop, startPhase, breatheScale, particleScale, playBtnScale]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.content, {
          ...
          <View style={styles.subtitleRow}>
            {PHASES.map((phase, i) => (
              <React.Fragment key={i}>
                <View style={[styles.subtitleDot, { backgroundColor: phase.accent }]} />
                <Text style={styles.subtitleVal}>{durations[i]}</Text>
                {i < 2 && <Text style={styles.subtitleSep}>·</Text>}
              </React.Fragment>
            ))}
          </View>
        </View>
            ...
            <Text style={styles.statusTextLarge}>
              {isRunning
                ? isPaused
                  ? 'paused'
                  : `${PHASES[activePhase].label.toLowerCase()}`
                : `${durations.reduce((a, b) => a + b, 0)}s × ${repetitions} = ${durations.reduce((a, b) => a + b, 0) * repetitions}s`}
            </Text>
          )}
        </View>
        ...
        {/* ── Settings row (only when not running) ─────── */}
        {!isRunning && (
          <View style={styles.settingsRow}>
            <TouchableOpacity
              style={[
                styles.settingsBtn,
                isEditing && { borderColor: `${COLORS.accent1}35`, backgroundColor: `${COLORS.accent1}0a` },
              ]}
              onPress={() => {
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.settingsBtnText, isEditing && { color: COLORS.accent1 }]}>
                {isEditing ? '✓ done' : '✎ edit'}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.settingsBtn,
                showRepEditor && { borderColor: `${COLORS.accent2}35`, backgroundColor: `${COLORS.accent2}0a` },
              ]}
              onPress={() => {
                setShowRepEditor(!showRepEditor);
                setIsEditing(false);
                if (Platform.OS !== 'web') Haptics.selectionAsync();
              }}
              activeOpacity={0.7}
            >
              <Text style={[styles.settingsBtnText, showRepEditor && { color: COLORS.accent2 }]}>
                ↻ {repetitions} {repetitions === 1 ? 'cycle' : 'cycles'}
              </Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {/* Repetition editor */}
        {showRepEditor && !isRunning && (
          <View style={styles.repEditor}>
            <TouchableOpacity
              onPress={() => { setRepetitions(Math.max(1, repetitions - 1)); if (Platform.OS !== 'web') Haptics.selectionAsync(); }}
              style={styles.repEditorBtn}
              activeOpacity={0.6}
            >
              <Text style={styles.repEditorBtnText}>−</Text>
            </TouchableOpacity>

            <View style={styles.repEditorValueBox}>
              <Text style={styles.repEditorValue}>{repetitions}</Text>
              <Text style={styles.repEditorLabel}>cycles</Text>
            </View>

            <TouchableOpacity
              onPress={() => { setRepetitions(Math.min(99, repetitions + 1)); if (Platform.OS !== 'web') Haptics.selectionAsync(); }}
              style={styles.repEditorBtn}
              activeOpacity={0.6}
            >
              <Text style={styles.repEditorBtnText}>+</Text>
...
const styles = StyleSheet.create({
  ...
  footerControls: {
    ...
    alignItems: 'center',
    gap: 16,
    marginTop: 10, // Reduced from 20 to save vertical space
  },
  playButton: {
    width: 60,
    height: 60,
